...
const Header: React.FC<HeaderProps> = ({ showProfile, notifications, onMarkAsRead }) => {
      ...
      {showProfile ? (
        <div className="flex items-center space-x-2 animate-in fade-in duration-300">
          <div className="w-10 h-10 rounded-full border-2 border-white shadow-sm overflow-hidden bg-white flex items-center justify-center">
            ...
            <img src={squirrelUrl} alt="Avatar" className="w-9 h-9 object-contain" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-gray-800 leading-none">이예진 · <span className="text-gray-400 font-medium text-sm">Lv2</span></h1>
          </div>
        </div>
      ) : (
                ...
                notifications.map((noti) => (
                  <div key={noti.id} className="px-4 py-3 border-b border-gray-50 last:border-none hover:bg-gray-50 transition-colors">
                    <div className="flex items-start space-x-3">
                      <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${noti.type === 'buy' ? 'bg-red-400' : 'bg-blue-400'}`}></div>
