    ...
    return (
      <div className="flex flex-col h-full bg-[#F4F8F6] relative animate-in fade-in duration-300">
        ...
        <div className="px-6 pb-4">
          ...
          <p className="text-sm text-gray-500 font-bold mt-2">중복 선택이 가능합니다.</p>
        </div>
            ...
            {agents.map((agent) => {
              ...
              return (
                <div 
                  ...
                  className={`relative rounded-[1.5rem] p-4 flex flex-col items-center text-center transition-all cursor-pointer border-2 ${
                    isSelected 
                      ? 'bg-white border-[#2D8C69] shadow-md scale-[1.02]' 
                ...
                >
                  <div className={`absolute top-3 left-3 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${
                    isSelected ? 'bg-[#2D8C69] border-[#2D8C69]' : 'bg-gray-100 border-gray-300'
                  }`}>
                    {isSelected && <Check size={14} className="text-white" />}
                  </div>
                  
                  <div className={`w-20 h-20 rounded-full ${agent.bgColor} mb-3 overflow-hidden border border-black/5`}>
                    <img 
                  ...
                  <h3 className="font-black text-gray-800 text-sm mb-1">{agent.name}</h3>
                  <p className="text-[10px] text-gray-500 font-bold break-keep leading-tight">
                    {agent.desc}
                  </p>
                </div>
              );
    ...
    <div className="flex flex-col h-full bg-[#E9EEF3] overflow-hidden font-['Pretendard'] animate-in slide-in-from-right duration-300">
            ...
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} items-end space-x-2`}>
              {!isUser && (
                <div className="flex flex-col items-center space-y-1 self-start">
                  <div className={`w-10 h-10 rounded-full ${agent?.bgColor || 'bg-white'} border border-gray-100 shadow-sm overflow-hidden flex items-center justify-center p-1`}>
                    <img 
      ...
      <div className="px-4 pt-2 pb-28 bg-gradient-to-t from-[#E9EEF3] via-[#E9EEF3] to-transparent shrink-0">
        <div className="bg-white rounded-full p-2 flex items-center shadow-lg border border-white">
          ...
          />
          <button onClick={handleFileClick} className="p-2 text-[#2D8C69] hover:bg-green-50 rounded-full transition-colors">
            <Plus size={24} />
          </button>
          <input 
            type="text" 
          ...
          <button 
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center shadow-md transition-all ${
              inputValue.trim() ? 'bg-[#2D8C69] text-white' : 'bg-gray-100 text-gray-300'
            }`}
          >
